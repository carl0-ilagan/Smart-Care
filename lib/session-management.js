...
export async function createOrUpdateSession(userId, userEmail) {
  try {
    ...
    // If user has 2FA enabled, check for suspicious login and notify
    try {
      const settingsSnap = await getDocDirect(docRefDirect(db, "userSettings", userId))
      const twoFactorEnabled = settingsSnap.exists() ? !!settingsSnap.data()?.security?.twoFactor : false
      // Only run suspicious check for truly new sessions (no prior record for this device+IP)
